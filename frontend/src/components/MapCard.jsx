  ...
  return (
    <Link to={`/map/${map.slug}`} className="group block">
      <Card className="overflow-hidden bg-gradient-to-br from-gray-900/50 to-gray-800/30 border-emerald-500/20 hover:border-emerald-400/50 transition-all duration-500 hover:shadow-2xl hover:shadow-emerald-500/20 hover:-translate-y-2 will-change-transform">
        {/* Image */}
        <div className="relative overflow-hidden aspect-[4/3] bg-gray-800" style={{backfaceVisibility: 'hidden'}}>
          {map.thumbnail ? (
            <>
              <img
              ...
              />
              <div 
                className="absolute top-0 left-0 right-0 bottom-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
